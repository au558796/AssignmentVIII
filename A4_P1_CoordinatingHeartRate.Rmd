...
convo1 = convo%>%
  ...
  dplyr::summarise(time=mean(time, na.rm=T),HR1=mean(scaleHR1, na.rm = T),HR2=mean(scaleHR2, na.rm = T),Resp1=mean(scaleResp1, na.rm = T),Resp2=mean(scaleResp2, na.rm = T))
...
rqa_analysis= function(x, y)(  
  crqa(x, y, embed = 2, delay = 24, normalize = 0, rescale = 0, radius = 033723308,
  mindiagline = 2, minvertline = 1)
)
rqa_analysis(convo1$HR1, convo1$HR2)
rqa_analysis(convo1$Resp1, convo1$Resp2)
rqa_analysis(convo1$HR1, convo1$Resp1)
rqa_analysis(convo1$HR2, convo1$Resp1)
...
for (file in file_list){
  ...
  condition = str_extract(str_extract(str_extract(file, "S\\w{4}\\d+[:punct:]G\\d+[:punct:]T\\d+"), "T\\d+"), "\\d+")
  name = paste(file)
...
#make loop for plotting
df <- result_df %>%
  group_by(name)%>%
  ggplot(df, aes(time, Resp1))+
    geom_line()+
    geom_line(aes(time, Resp2), color = 'Red')
  # 
  # plot2<-ggplot(df, aes(time, HR1))+
  #   geom_line()+
  #   geom_line(aes(time, HR2), color = 'Red')
  # 
  # plot3<-ggplot(df, aes(time, HR1))+
  #   geom_line()+
  #   geom_line(aes(time, Resp1), color = 'Red')
  # 
  # plot4<-ggplot(df, aes(time, HR2))+
  #   geom_line()+
  #   geom_line(aes(time, Resp2), color = 'Red')
...
names <- as.vector(unique(result_df['name']))
names <- paste(unlist(names))
